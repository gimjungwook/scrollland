...
function renderSection(section, i, lesson, motion, heading) {
 ...
 if (section.kind === 'reading') {
   return `<section class="reading-section" id="${e(section.id)}" data-kind="reading">${opening}${sourceInput(section)}${section.code ? example(section, lesson) : ''}${staticDiagram(section)}${section.trace ? `<ol class="static-trace">${section.trace.map(step=>`<li><strong>${e(step.label)}</strong><p>${e(step.state)}</p></li>`).join('')}</ol>` : ''}</section>`;
 }
...
function article(lesson, content, prefix='', single=false) {
 const heading=single?'h1':'h2', sectionHeading=single?'h2':'h3';
 const course=content.curriculum.courses.find(c=>c.id===lesson.courseId), index=content.order.indexOf(lesson.slug);
 return `<article class="lesson-article" id="lesson-${lesson.slug}" data-lesson-article data-title="${e(lesson.title)}" data-content-fingerprint="${fingerprint(lesson)}"><header class="lesson-opening"><p class="eyebrow">${n(index+1)} / ${n(content.order.length)} · ${e(course.title)}</p><${heading}>${e(lesson.title)}</${heading}><p class="opening-problem">${e(lesson.problem)}</p><div class="opening-bottom"><p>${e(lesson.goal)}</p></div>${lesson.prerequisites.length?`<p class="prerequisites">먼저 익힐 개념 · ${lesson.prerequisites.map(slug=>`<a href="${prefix}index.html#lesson-${slug}">${e(content.lessons[slug].title)}</a>`).join(' · ')}</p>`:''}</header>${lesson.sections.map((section,i)=>renderSection(section,i,lesson,content.motion?.scenes?.[section.id],sectionHeading)).join('')}${renderQuiz(lesson,sectionHeading)}<section class="lesson-takeaway"><p class="eyebrow">기억할 한 가지</p><${sectionHeading}>${e(lesson.takeaway)}</${sectionHeading}><div class="source-links">${lesson.sourceUrls.map((url,i)=>`<a href="${e(url)}">Python 공식 자료 ${i+1} ↗</a>`).join('')}</div></section></article>`;
}
function catalog(content) {
 return `<section class="catalog" id="courses"><p class="eyebrow">전체 학습 지도</p><h2>44개의 문제.<br>하나로 이어지는 이해.</h2>${content.curriculum.courses.map((course,i)=>`<div class="course"><span class="course-index">${n(i+1)}</span><div><h3>${e(course.title)}</h3><p>${e(course.question)}</p><ol>${course.lessons.map(slug=>`<li><a href="#lesson-${slug}"><span>${n(content.order.indexOf(slug)+1)}</span>${e(content.lessons[slug].title)}<i aria-hidden="true">↗</i></a></li>`).join('')}</ol></div></div>`).join('')}</section>`;
}
export function renderIndex(content) {
 return frame('한 줄을 읽고, 변화를 이해하다','글과 예제를 읽고 필요한 곳에서 변화 과정을 살펴보는 Python 학습 여정.',`<main id="main">${hero()}${content.order.map(slug=>article(content.lessons[slug],content)).join('')}${catalog(content)}</main>`);
}
