...
export function validateLesson(lesson) {
  ...
  lesson.sections.forEach((section, i) => {
    ...
    if ('diagram' in section) {
      ...
      section.diagram.nodes.forEach(node => { object(node, p); string(node.label, `${p}.label`); string(node.value, `${p}.value`); if ('objectId' in node) string(node.objectId, `${p}.objectId`); });
    }
    if ('trace' in section) {
